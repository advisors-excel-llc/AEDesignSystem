...
const FilterField = props => {
  const [predicate, setPredicate] = useState('is empty')
  const {filters, removeFilter, setFilter, components = [], filter, properties, ...rest} = props
  ...
  if (null !== filter.value && typeof filter.value !== 'undefined' && !!valueField && typeof valueField.type.buildFieldPredicate === 'function') {
    ...
    if (newPredicate !== predicate) setPredicate(newPredicate)
  }
  ...
  return (filter.isNew
    ...
    >
      ...
      <div className="slds-p-around--small">
        <Provider value={{predicate, setPredicate, filters, removeFilter, setFilter, filter, properties, ...rest}}>
          {valueField}
        </Provider>
      </div>
    </FilterInput>
